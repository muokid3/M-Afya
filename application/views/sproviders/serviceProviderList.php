...
function vr(nm,id){
  bootbox.confirm("Are you sure you want to Suspend " + nm + "?", function(result) {
      if(result) {
      
	  $.ajax({
		url: '<?php echo base_url(); ?>Service_providers/actionSuspend/' + id,
		type: 'post',
		data: {id: id},
		dataType: 'html',		
		success: function(html) {
				bootbox.alert(html);
		    if(html == 'Service Provider Suspended Successfully')
				{
					location.reload();
				}
		}
					...
					<h4 style="color:#E01FEB"><i class="fa fa-medkit"></i> <strong>Verified Service Providers</strong></h4>
							  ...
							  <?php  
								...
								<tr>
									  ...
									  <td>
										...
										<a href="javascript:void(0);" onclick="vr('<?php echo $sprovider->bs_name; ?>','<?php echo $sprovider->id; ?>');">
										 	<span style="width:100%; margin-bottom:5px" class="btn btn-warning"><span class="fa fa-unlink"></span>&nbsp;Suspend</span></a>
										 	<br>

										 <a href="javascript:void(0);" onclick="rm('<?php echo $sprovider->bs_name; ?>','<?php echo $sprovider->id; ?>');">
										 	<span style="width:100%" class="btn btn-danger"><span class="glyphicon glyphicon-trash"></span>&nbsp;Delete</span></a>
