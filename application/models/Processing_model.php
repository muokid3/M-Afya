...
class Processing_model extends CI_Model {
    ...
    function get_list_processing($searchvalue = null){
   
		if ($searchvalue == null) 
		{
			$this->db->select('merchant_settlements.*, merchant_settlements.id AS ms_id, merchants.*, merchants.id AS m_id');
			$this->db->from('merchant_settlements');
			$this->db->join('merchants', 'merchant_settlements.business_no = merchants.business_no');
			$this->db->where('merchant_settlements.settlement_status', 2);
		...
		else
		{
			...
			$another_where = "merchants.bs_name LIKE '%".$searchvalue."%' AND merchant_settlements.business_no = merchants.business_no AND merchant_settlements.settlement_status = 2";
	...
	function suspend($id, $suspension)
	{
		$this->db->where('id', $id);
		$this->db->update($this->merchant_settlements, $suspension);
	}

	function accept($id, $acceptance)
	{
		$this->db->where('id', $id);
		$this->db->update($this->merchant_settlements, $acceptance);
	}

	function decline($id, $declination)
	{
		$this->db->where('id', $id);
		$this->db->update($this->merchant_settlements, $declination);
	}
}
?>