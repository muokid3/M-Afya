<?php
class Suspended_model extends CI_Model {
	...
	function get_list_suspended($searchvalue = null)
	{
		if ($searchvalue == null) 
		{
			...
			$this->db->where('merchant_settlements.settlement_status', 3);
		...
		else
		{
			...
			$another_where = "merchants.bs_name LIKE '%".$searchvalue."%' AND merchant_settlements.business_no = merchants.business_no AND merchant_settlements.settlement_status = 3";
		...
		//return $this->db->where("settlement_status = 3")->select('*, merchant_settlements.business_no as business_no, merchant_settlements.id as id, merchants.bs_name as merchants')->from($this->merchant_settlements)->join('merchants', 'merchants.business_no = merchants.business_no', 'left')->get();
    }
	...
	function unsuspend($id, $suspension)
	{
		$this->db->where('id', $id);
		$this->db->update($this->merchant_settlements, $suspension);
	}
}
?>